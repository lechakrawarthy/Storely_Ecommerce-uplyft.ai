...
def chat_endpoint():
    ...
    if 'hello' in user_message or 'hi' in user_message:
        response['message'] = "Hello! I'm your Book Buddy AI assistant. How can I help you find a book today?"
        response['suggestions'] = ['Show me fiction books', 'Science books', 'History books']
    elif 'fiction' in user_message:
        db = SessionLocal()
        try:
            books = db.query(Product).filter(Product.category == 'Fiction').limit(5).all()
        ...
        db = SessionLocal()
        try:
            books = db.query(Product).filter(Product.category == 'Science').limit(5).all()
        ...
        db = SessionLocal()
        try:
            books = db.query(Product).filter(Product.category == 'History').limit(5).all()
        ...
        response['suggestions'] = ['Fiction books', 'Science books', 'History books']
